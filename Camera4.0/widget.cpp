    ...
    , ui(new Ui::Widget)
{
    ...
    UIUpdater *updata = new UIUpdater;

    connect(this, &Widget::starting, updata, &UIUpdater::working);
    emit starting(); //让'updata'对象在线程里运行起来
    ...
    connect(updata,     &UIUpdater::updateUI,      this, &Widget::on_displayReceiveMsg);
...
{
    mMyUdpCast->write("[from "+ getLocalIP()+"] "+ui->ProtocolCmd->text());
}
...
/*--------------@brief：接收'send'函数发送成功信号，在'QPlainTextEdit'里显示消息-------------*/
void Widget::on_displayReceiveMsg(const QString& data)
{
    //ui->plainTextEdit->appendPlainText("[from "+ getLocalIP()+"] "+data);
