...
EnHandleResult MyUdpMultiCastListener::OnReceive(IUdpCast* pSender, CONNID dwConnID, const BYTE* pData, int iLength)
{
    ...
    //'invokeMethod'可以在不同线程之间调用对象的方法,此处调用'mythread'线程里的'receiveDataFromUdp'槽函数
    QMetaObject::invokeMethod(updata, "receiveDataFromUdp", Qt::QueuedConnection, Q_ARG(QString, strData));
    qDebug() <<"OnReceive";
    return HR_OK;
}
...
void MyUdpCast::write(const QString &data)
{
    ...
    int dataSize = byteArray.size(); //获得实际数据大小

    bool ret = pUdpCast->Send(pBuffer, dataSize);
