    ...
    , ui(new Ui::Widget)
{
    ...
    /*t1 = new QThread;
    udpdater = new UIUpdater;
    udpdater->moveToThread(t1);*/
    ...
    //connect(myListener, SIGNAL(receiveComplete()), this, SLOT(on_displayCloseMsg()));
    //connect(myListener, &MyUdpMultiCastListener::receiveComplete, this, &Widget::on_displayReceiveMsg);
    connect(udpdater,   &UIUpdater::updateUI, this, &Widget::on_displayReceive);
...
/*--------------@brief：接收'send'函数发送成功信号，在'QPlainTextEdit'里显示消息-------------*/
void Widget::on_displayReceiveMsg(QString data)
{
...
//    UIUpdater *udpdater = new UIUpdater;
//    udpdater->moveToThread(t1);

    //t1->start();
    emit startDataProcessing(data);
    qDebug() << "66";
}

void Widget::on_displayReceive(QString data)
{
    qDebug() << "88";
}
